...
static int open_collection(struct hid_parser *parser, unsigned type)
{
	...
	INIT_LIST_HEAD(&collection->children);
	collection->parent = parser->active_collection;
	if (collection->parent)
		list_add_tail(&collection->list, &collection->parent->children);
					  ...
					  int effective_multiplier)
{
	 ...
	 * Otherwise, it is the collection the multiplier applies to but any
	 * field may be in a subcollection of that collection.
	 */
				 ...
				 struct hid_field *multiplier)
{
	...
	while (multiplier_collection && multiplier_collection->type != HID_COLLECTION_LOGICAL)
